...
template<typename GradePolicyFnT>
double
gradeAnalysis(const vector<StudentInfoT>& students, GradePolicyFnT gpf) {
    vector<double> grades;
    transform(students.begin(), students.end(), back_inserter(grades), gpf);
    return median(grades);
}

template<typename GradePolicyFnT>
void
writeAnalysis(ostream& os, const string& type, GradePolicyFnT gpf,
    const vector<StudentInfoT>& did, const vector<StudentInfoT>& didnot) {
    os << endl;
    os << type << " : median(did) : " << gradeAnalysis(did, gpf) << endl;
    os << type << " : median(didnot) : " << gradeAnalysis(didnot, gpf) << endl;
}
