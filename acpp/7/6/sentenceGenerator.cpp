...
genAux(const GrammarT& g, const string& word, list<string>& sentence) {
    // Add the given word to rules stack
    vector<string> rulesStack;
    rulesStack.push_back(word);

    // Continue popping off and expanding the contents of
    // the rule stack till it is not empty
    while (!rulesStack.empty()) {
        // Pop off a word from the rule stack
        string w = rulesStack.back();
        rulesStack.pop_back();

        if (!bracketed(w)) {
            // Popped word not a rule/category; so added to sentence
            // without any expansion
            sentence.push_back(w);

        } else {

            // Popped word is a rule/category; so requires lookup
            // in grammar table followed by rule/category expansion
            GrammarT::const_iterator iter = g.find(w);
            ...
            // Add the rule to the rule stack in order to perform
            // rule expansion for the selected rule by unwinding
            // of rules stack contents
            for (RuleT::const_reverse_iterator rIter = r.rbegin();
                rIter != r.rend(); ++rIter) {
                rulesStack.push_back(*rIter);
            }
        }
    }
}
