...
public:
    ...
    // Inserts the chars in the range [b, e) into the string
    // from the iterator location commencing at i
...
Str::Str(In b, In e) :
    vdata(b, e), strDataBuf(NULL), dataBuf(NULL) {
}
...
template<typename InputIterator>
void
Str::insert(iterator i, InputIterator b, InputIterator e) {
    vdata.insert(i, b, e);
}
