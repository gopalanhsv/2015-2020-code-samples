...
public:
    ...
    // Replaces the elements in the vector with the elements
    // in the range [b, e) denoted by input iterators
    template<typename InputIterator>
...
template<typename InputIterator>
void
Vec<T>::assign(InputIterator b, InputIterator e) {
    // Remove the existing vector elements
    uncreate();
    // Append the elements in the range [b, e)
    append(b, e);
}
