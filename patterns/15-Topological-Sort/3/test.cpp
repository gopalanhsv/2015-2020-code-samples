...
test() {
    ...
    try {
        vector<unsigned> orderedTasksV =
            TaskScheduler::scheduleTasks(nt, preReqV);
        if (orderedTasksV.empty()) {
            cout << "Task scheduling not possible due to cyclic dependency"
                <<  " of prerequisites!" << endl;
        } else {
            cout << "Task schedule order as per prerequisites : ";
            printVec(orderedTasksV);
        }
